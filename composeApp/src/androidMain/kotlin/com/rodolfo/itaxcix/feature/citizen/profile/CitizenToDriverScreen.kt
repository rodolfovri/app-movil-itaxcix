...
@Preview
@Composable
fun CitizenToDriverScreenPreview() {
    CitizenToDriverScreen()
}

@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun CitizenToDriverScreen(
    viewModel: CitizenProfileViewModel = hiltViewModel(),
    onBackPressed: () -> Unit = {},
    onSuccess: () -> Unit = {}
) {
    // Recolectar estados
    val convertToDriverState by viewModel.convertToDriverState.collectAsState()
    val isLoading = convertToDriverState is CitizenProfileViewModel.ConvertToDriverState.Loading
    val isSuccess = convertToDriverState is CitizenProfileViewModel.ConvertToDriverState.Success
    ...
    LaunchedEffect(key1 = convertToDriverState) {
        when (val state = convertToDriverState) {
            is CitizenProfileViewModel.ConvertToDriverState.Error -> {
                isSuccessSnackbar = false
                snackbarHostState.showSnackbar(
                    message = state.message,
                    duration = SnackbarDuration.Short
                )
                viewModel.onConvertToDriverErrorShown()
            }
            is CitizenProfileViewModel.ConvertToDriverState.Success -> {
                isSuccessSnackbar = true
                delay(2000)
                onSuccess()
                viewModel.onConvertToDriverSuccessShown()
            }
            else -> {}
        }
    }
            ...
            Box(
                ...
                Column(
                    ...
                    Column(
                        ...
                        Text(
                            text = "Ingresa la placa de tu vehículo para comenzar tu registro como conductor.",
                            modifier = Modifier
                                .fillMaxWidth()
                        ...
                        Button(
                            ...
                            Text(
                                text = "Solicitar conversión a conductor",
                    ...
                    Text(
                        text = buildAnnotatedString {
                            append("Al continuar, tu solicitud será revisada por nuestro equipo. ")
                            withStyle(
                                style = MaterialTheme.typography.labelLarge.toSpanStyle()
                                    .copy(color = ITaxCixPaletaColors.Blue1)
                            ) {
                                append("Te notificaremos el resultado.")
                            }
                        },
                        modifier = Modifier
                            .fillMaxWidth()
