...
fun DriverHistoryScreen(
    ...
    Box(
        ...
        when (historyState) {
            is DriverHistoryViewModel.HistoryState.Loading -> {
                Box(
            ...
            is DriverHistoryViewModel.HistoryState.Error -> {
                val errorState = historyState as DriverHistoryViewModel.HistoryState.Error
                Box(
                    modifier = Modifier.fillMaxSize(),
                    contentAlignment = Alignment.Center
                ) {
            ...
            is DriverHistoryViewModel.HistoryState.Success -> {
                val travelHistory = (historyState as DriverHistoryViewModel.HistoryState.Success).data.data
...
fun StatusChip(status: String) {
    val (backgroundColor, textColor) = when (status) {
        ...
        "CANCELADO" -> Pair(Color.Red.copy(alpha = 0.1f), Color.Red)
        "COMPLETADO" -> Pair(Color.Green.copy(alpha = 0.1f), Color.Green)
        "EN_PROGRESO" -> Pair(Color.Yellow.copy(alpha = 0.1f), Color.Yellow)
